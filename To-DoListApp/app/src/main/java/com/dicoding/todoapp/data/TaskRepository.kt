...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        ...
        val getQuery = FilterUtils.getFilteredQuery(filter)
        val configuration = PagedList.Config.Builder().setEnablePlaceholders(PLACEHOLDERS).setPageSize(
            PAGE_SIZE).setInitialLoadSizeHint(PAGE_SIZE).build()
        return LivePagedListBuilder(tasksDao.getTasks(getQuery), configuration).build()
    }
